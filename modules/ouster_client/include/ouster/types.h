...
/*
 * Constants used for configuration. Refer to the sensor documentation
 ...
 * 
 * @param lidar_mode
 * @return default sensor_info for the OS1-32
 */
 ...
 * 
 * @param lidar_info
 * @return string representation og the lidar mode or "UNKNOWN"
 */
 ...
 */
uint32_t n_cols_of_lidar_mode(lidar_mode);
 ...
 */
int frequency_of_lidar_mode(lidar_mode);
 ...
 * 
 * @throw runtime_error id the text is not valid json
 * @param metadata a text blob returned by get_metadata from client.h
 * @return a sensor_info struct populated with a subset 
 */
...
/**
 * Get sring representation of metadata.
 * 
 ...
 * 
 * In the user guide, refer to section 9 for the lidar packet format
 * and section 10 for imu packets.
 * 
 * For 0 <= n < columns_per_packet, nth_measurement_block(n, packet_buf) returns a pointer to
 * the nth measurement block. For 0 <= m < pixels_per_column, nth_channel(m, measurement_block_buf)
 * returns the mth channel data block.
 * 
 * Use imu_la_{x,y,z} to access the accleration in the corresponding
 * direction. Use imy_av_{x,y,z} to read the angular velocity
 */
struct packet_format{
    ...
    const uint8_t* (*const nth_measurement_block)(int n, const uint8_t* lidar_buf);
    uint64_t (*const measurement_block_timestamp)(const uint8_t* measurement_block_buf);
    uint32_t (*const measurement_block_encoder)(const uint8_t* measurement_block_buf);
    uint16_t (*const measurement_block_measurement_id)(const uint8_t* measurement_block_buf);
    uint16_t (*const measurement_block_frame_id)(const uint8_t* measurement_block_buf);
    uint32_t (*const measurement_block_status)(const uint8_t* measurement_block_buf);

    // Channel data block accessors
    const uint8_t* (*const nth_channel)(int n, const uint8_t* measurement_block_buf);
    uint32_t (*const channel_range)(const uint8_t* channel_buf);
    uint16_t (*const channel_reflectivity)(const uint8_t* channel_buf);
    uint16_t (*const channel_signal)(const uint8_t* channel_buf);
    uint16_t (*const channel_infrared)(const uint8_t* channel_buf);
